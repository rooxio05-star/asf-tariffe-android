...
#
##############################################################################
# --- ensure gradle wrapper jar exists (GitHub Actions / fresh checkout) ---
WRAPPER_JAR="$APP_HOME/gradle/wrapper/gradle-wrapper.jar"
WRAPPER_PROPS="$APP_HOME/gradle/wrapper/gradle-wrapper.properties"

if [ ! -f "$WRAPPER_JAR" ] && [ -f "$WRAPPER_PROPS" ]; then
  DISTR_URL=$(grep -E '^distributionUrl=' "$WRAPPER_PROPS" | cut -d'=' -f2- | sed 's/\\:/:/g')
  # Download Gradle distribution and extract wrapper jar
  TMP_DIR=$(mktemp -d)
  curl -L "$DISTR_URL" -o "$TMP_DIR/gradle.zip"
  unzip -q "$TMP_DIR/gradle.zip" -d "$TMP_DIR/gradle"
  # Find wrapper jar inside distribution
  FOUND_JAR=$(find "$TMP_DIR/gradle" -name "gradle-wrapper-*.jar" | head -n 1)
  if [ -n "$FOUND_JAR" ]; then
...
# --- end ensure gradle wrapper jar exists ---
